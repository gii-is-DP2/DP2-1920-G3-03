...
class HU25_RoutineLineManagementByTrainer extends Simulation {
  ...
  setUp(
    UpdateRoutineLineByTrainerSuccessfulScn.inject(rampUsers(3000) during (100 seconds)),
    UpdateRoutineLineByTrainerErrorScn.inject(rampUsers(1000) during (100 seconds))).protocols(httpProtocol)
    .assertions(
