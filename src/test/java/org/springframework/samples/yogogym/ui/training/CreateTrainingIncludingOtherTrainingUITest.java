...
public class CreateTrainingIncludingOtherTrainingUITest {
  ...
  public void testCreateTrainingInitInTrainingUI() throws Exception {
    ...
    int end2 = cal.get(Calendar.DAY_OF_MONTH);
    driver.findElement(By.linkText(String.valueOf(end2))).click();;
    driver.findElement(By.xpath("//button[@type='submit']")).click();
    try {
      assertEquals("The training cannot be in a period which includes another training (The other training is from " + initString + " to " + endString + ")", driver.findElement(By.xpath("//form[@id='trainingForm']/div/div[3]/div/span[2]")).getText());
    } catch (Error e) {
      verificationErrors.append(e.toString());
    }
    driver.findElement(By.linkText("Trainer")).click();
