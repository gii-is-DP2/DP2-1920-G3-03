...
public class CreateTrainingInitInTrainingUITest {
  ...
  public void testCreateTrainingInitInTrainingUI() throws Exception {
    ...
    driver.findElement(By.xpath("//button[@type='submit']")).click();
    try {
      assertEquals("The training cannot start in a period with other training (The other training is from " + initString + " to " + endString + ")", driver.findElement(By.xpath("//form[@id='trainingForm']/div/div[2]/div/span[2]")).getText());
    } catch (Error e) {
      verificationErrors.append(e.toString());
    }
    driver.findElement(By.linkText("Trainer")).click();
